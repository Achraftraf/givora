...
// Mutation to handle user message
const { mutate } = useMutation({
mutationFn: (message) => generateChatResponse(message),
...
// Function to fetch the AI's response after sending the user's message
...
setMessages((prevMessages) => [
...prevMessages,
{ text: answer.message, type: "bot" }, // Add AI's response
]);
} catch (error) {
...
// Handle form submission for the user's message
const handleSubmit = async (e) => {
e.preventDefault();
if (text.trim()) {
// Send the user's message and fetch AI's response
mutate(text, {
onSuccess: () => {
getAiResponse(text);
},
...
// Function to render messages that may contain code blocks
const renderMessage = (msgText) => {
const codeBlockRegex = /`([\s\S]*?)`/g;
const parts = msgText.split(codeBlockRegex); // Split by code blocks

    return parts.map((part, index) => {
      // Check if it's a code block or normal text
      if (index % 2 === 1) {
        // It's a code block (odd indexes are code blocks)
        const lines = part.trim().split("\n"); // Split into lines
        const language = lines[0]; // The first line is the language name
        const code = lines.slice(1).join("\n"); // The rest is the code

        return (
          <div key={index} className="code-block">
            <div className="language-tag bg-gray-800 text-gray-100 px-2 py-1 rounded-t-md font-medium">
              {language}
            </div>
            <pre className="whitespace-pre-wrap text-sm font-mono bg-gray-900 text-lime-400 p-4 rounded-b-md shadow-inner">
              <code>{code}</code>
            </pre>
          </div>
        );
      } else {
        // Normal text (even indexes are normal text)
        return <p key={index} className="text-md">{part}</p>;
      }
    });
...
What coding challenges are you facing?
</h2>
<div className="h-[65dvh] overflow-y-auto flex flex-col gap-6 scrollbar-hidden p-4">
{messages.map((msg, index) => (
<div
key={index}
className={`max-w-[75%] p-4 rounded-3xl w-fit shadow-md transition-all duration-300 ${
                msg.type === "user"
                  ? "self-end bg-gray-100 text-gray-900 rounded-br-none"
                  : "self-start bg-gray-200 text-gray-800 rounded-bl-none"
              }`} >
{renderMessage(msg.text)}
</div>
))}
<div ref={messagesEndRef} /> {/_ Empty div for scrolling _/}
</div>
</div>
<form
        onSubmit={handleSubmit}
        className="w-full max-w-2xl mx-auto py-4 px-6 rounded-lg"
      >
<div className="flex w-full items-center space-x-4">
<input
type="text"
placeholder="Chat with AI"
className="flex-grow p-3 border border-gray-300 rounded-full shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-500 transition duration-200"
value={text}
required
onChange={(e) => setText(e.target.value)}
/>
<button
            className="bg-gray-800 text-white px-6 py-3 rounded-full shadow-md focus:outline-none focus:ring-2 focus:ring-gray-500 flex items-center justify-center transition duration-200"
            type="submit"
          >
...
cod with th dark mode

import { Prism as SyntaxHighlighter } from "react-syntax-highlighter";
import { dark } from "react-syntax-highlighter/dist/esm/styles/prism";

// In your renderMessage function, use the dark theme
<SyntaxHighlighter
key={index}
language={language.toLowerCase()}
style={dark} // Change to dark theme
